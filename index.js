...
app.get('/states/highest-gdp', (req, res) => {
  const highest = states.reduce((prev, current) => (prev.gdp > current.gdp) ? prev : current);
...
app.put('/states/:id', (req, res) => {
  ...
  states[index] = { id: parseInt(req.params.id), ...req.body };
...
app.delete('/states/low-literacy/:percentage', (req, res) => {
  ...
  states = states.filter(s => s.literacyRate >= parseFloat(req.params.percentage));
  res.status(200).json({ deletedCount: initialCount - states.length });
});

app.delete('/states/name/:stateName', (req, res) => {
  const nameToMatch = req.params.stateName.toLowerCase();
  const initialCount = states.length;
  states = states.filter(s => s.name.toLowerCase() !== nameToMatch);
  
  if (states.length === initialCount) return res.status(404).json({ message: "State not found" });
  res.status(204).send();
});
